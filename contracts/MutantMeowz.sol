...
contract MutantMeowz is ERC721Enumerable, Ownable {
    using Strings for uint256;
    event MintCat(address indexed sender, uint256 startWith, uint256 times);

    //supply counters 
    uint256 public totalMeowz;
    ...
    function mint(uint256 _times) payable public {
        ...
        require(totalMeowz + _times <= totalCount, "max supply reached!");
        require(msg.value == _times * price, "value error, please check price.");
        payable(owner()).transfer(msg.value);
        emit MintCat(_msgSender(), totalMeowz+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalMeowz++);
        }
    }  
}