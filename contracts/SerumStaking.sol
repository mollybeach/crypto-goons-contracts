...
contract SerumStaking is ERC1155, IERC721Receiver, Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public foxFor25Boost = 10 ether;
    uint256 public foxFor50Boost = 25 ether;
    uint256 public foxFor100Boost = 50 ether;
    ...
    function setFoxBoostAmts(uint256 boost25, uint256 boost50, uint256 boost100) external onlyOwner {
        foxFor25Boost = boost25;
        foxFor50Boost = boost50;
        foxFor100Boost = boost100;
    }

    function feedGorilla(uint256 tokenId, uint256 foxAmt) external {
        _tokenRewards[tokenId].foxDeposited += foxAmt;
        erc20Token.transferFrom(_msgSender(), address(this), foxAmt);
    }
    ...
    function claimSerum() external whenNotPaused {
        ...
        for (uint256 i = 0; i < _deposits[_msgSender()].length(); i++) {
            ...
            if(serum.foxDeposited >= foxFor100Boost) {
            ...
            else if(serum.foxDeposited >= foxFor50Boost) {
            ...
            else if(serum.foxDeposited >= foxFor25Boost) {
            ...
            if(serum.startTime > 0 && block.timestamp >= serumCreationTime) {
                ...
                if(serum.foxDeposited > foxFor100Boost) {
                    serum.foxDeposited -= foxFor100Boost;
                }
                else {
                    serum.foxDeposited = 0;
                }
            }
        }
