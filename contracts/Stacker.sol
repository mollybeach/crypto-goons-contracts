...
contract Stacker is ERC721URIStorage, Ownable, IERC721Receiver {
    ...
    address public unstackedPupAddress;
    ...
    constructor(string memory name_, string memory symbol_, address _unstackedAddress, string memory baseURI_) ERC721(name_, symbol_) {
        unstackedPupAddress = _unstackedAddress;
        baseURI = baseURI_;
    }
    ...
    function devMint(uint256 _times) public onlyOwner {
        emit MintStack(_msgSender(), totalPupz+1);
    ...
    function stack(uint256[] calldata _tokenIds) public {
        ...
        require(IERC721(unstackedPupAddress).isApprovedForAll(_msgSender(), address(this)), "unstackedPupz not approved for spending");
        require(_tokenIds.length == 3, "you need 3 unstacked pups to stack");
        require(totalPupz + 1 <= totalCount, "not enough pupz");
        for (uint256 i; i < _tokenIds.length; i++) {
          IERC721(unstackedPupAddress).safeTransferFrom(_msgSender(), address(this), _tokenIds[i]);
        }
        emit MintStack(_msgSender(), totalPupz+1); //emit a MintStackEvent
        _mint(_msgSender(), 1 + totalPupz++);
    }
